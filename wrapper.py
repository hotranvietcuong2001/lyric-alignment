...
def preprocess_from_file(audio_file, lyrics, word_file=None):
    y, sr = preprocess_audio(audio_file)

    words, lyrics_p, idx_word_p, idx_line_p = preprocess_lyrics(lyrics, word_file)
...
def preprocess_lyrics(lyrics, word_file=None):
    ...
    raw_lines = ["".join([c for c in line.lower() if c in d.keys()]).strip() for line in [lyrics]]
